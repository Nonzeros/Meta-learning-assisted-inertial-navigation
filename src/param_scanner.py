...
def generate_param_combinations(
    scan_config: Dict[str, Any], base_config: Dict[str, Any]
) -> List[Dict[str, Any]]:
    """
    生成所有参数组合
    ...
    if scan_mode == "auto":
        ...
        if scan_filter_type:
            ...
            for lambda1, Q, R in itertools.product(
                kf_lambda1_values, kf_Q_values, kf_R_values
            ):
        ...
        for param_combo in param_combinations:
            ...
            # 更新粒子滤波参数
            if (
                "particle_filter" in param_combo
                and param_combo["particle_filter"] is not None
            ):
            ...
            # 更新卡尔曼滤波参数
            if (
                "kalman_filter" in param_combo
                and param_combo["kalman_filter"] is not None
            ):
        ...
        if filter_type == 2:
            ...
            lambda1_values = pf_scan.get(
                "lambda1_values", [base_config.get("filter", {}).get("lambda1", 0.1)]
            )
            numPar_values = pf_scan.get(
                "numPar_values", [base_config.get("filter", {}).get("numPar", 30)]
            )

            # 生成所有粒子滤波参数组合（lambda1, numPar, R, q0）
            for lambda1, numPar, R, q0 in itertools.product(
                lambda1_values, numPar_values, R_values, q0_values
            ):
                new_config = copy.deepcopy(base_config)
                new_config["filter"]["lambda1"] = lambda1
...
def _generate_ukf_combinations(
    scan_config: Dict[str, Any], base_config: Dict[str, Any]
) -> List[Dict[str, Any]]:
    """
    生成MATLAB UKF参数组合
