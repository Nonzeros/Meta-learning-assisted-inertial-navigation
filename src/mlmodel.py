...
## 实时计算a
def real_time_adaptation(data_num,a0,P0,Q,R,phi_net, h_net, input_data, label_data, lambda1, vel_data, pos_data):
...
## 可以时时计算a，从而估计气动力的代码
def validation2(phi_net, h_net, adaptinput: np.ndarray, adaptlabel: np.ndarray, valinput: np.ndarray,vallabel: np.ndarray, pos_data,options, lambda1,lam=0):
    with torch.no_grad():
        # 适应阶段最小二乘计算 a 的初始值
        ...
        for i in range(0,len(valinput),span):
            Y_bar,a0,P0 = real_time_adaptation(span,dim_a,a0,P0,Q,R,phi_net,h_net,valinput[i,:],vallabel[i,:],lambda1,pos_data[i,:])
            Y_bars[i,:] = Y_bar
