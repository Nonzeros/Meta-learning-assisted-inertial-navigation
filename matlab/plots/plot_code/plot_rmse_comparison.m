...
for file_idx = 1:size(file_config, 1)
    ...
    % 零气动力（Baseline 模型闭环速度）
    baseline_vx = data.baseline_ukf_fused_vx(valid_idx);
    baseline_vy = data.baseline_ukf_fused_vy(valid_idx);
    baseline_vz = data.baseline_ukf_fused_vz(valid_idx);
    ...
    % 零气动力
    rmse_baseline_x = sqrt(mean((baseline_vx - real_vx).^2));
    rmse_baseline_y = sqrt(mean((baseline_vy - real_vy).^2));
    rmse_baseline_z = sqrt(mean((baseline_vz - real_vz).^2));
    ...
    % 组织数据用于绘图
    % 行：三个方向（东、北、天）
    % 列：四种方法（元学习、零气动力、线性阻力、纯惯导）
    rmse_data = [
        rmse_meta_x,   rmse_baseline_x,   rmse_linear_x,   rmse_pure_x;    % 东向
        rmse_meta_y,   rmse_baseline_y,   rmse_linear_y,   rmse_pure_y;    % 北向
        rmse_meta_z,   rmse_baseline_z,   rmse_linear_z,   rmse_pure_z     % 天向
    ];
    
    % 绘制分组柱状图（略窄一些，便于排版）
    figure('Position', [100, 100, 800, 500], 'Name', 'RMSE对比');
    
    % 创建分组柱状图
    b = bar(rmse_data);
    
    % 设置柱子颜色 - 使用科研论文级配色方案
    % 参考Nature/Science期刊常用配色，色盲友小且高对比度
    b(1).FaceColor = [0.12, 0.47, 0.71];    % 元学习 - 深蓝色
    b(2).FaceColor = [0.60, 0.60, 0.60];    % 零气动力 - 中灰色
    b(3).FaceColor = [0.89, 0.47, 0.20];    % 线性阻力 - 暖橙色
    b(4).FaceColor = [0.20, 0.63, 0.17];    % 纯惯导 - 深绿色
    
    % 设置柱子边框，增强视觉效果
    b(1).EdgeColor = [0.10, 0.40, 0.65];
    b(2).EdgeColor = [0.50, 0.50, 0.50];
    b(3).EdgeColor = [0.80, 0.42, 0.18];
    b(4).EdgeColor = [0.18, 0.55, 0.15];
    b(1).LineWidth = 1.2;
    b(2).LineWidth = 1.2;
    b(3).LineWidth = 1.2;
    b(4).LineWidth = 1.2;
    ...
    % 设置横轴标签
    set(gca, 'XTickLabel', {'东向', '北向', '天向'}, 'FontName', 'SimSun', 'FontSize', 17);
    
    % 设置纵轴标签和标题
    ylabel('RMSE (m/s)', 'FontName', 'Times New Roman', 'FontSize', 18);
    title(sprintf('三方向速度RMSE对比 - %s', wind_title), 'FontName', 'SimSun', 'FontSize', 20);
    
    % 添加图例（元学习、零气动力、线性阻力、纯惯导）
    legend({'元学习模型', '零气动力模型', '线性阻力模型', '纯惯导'}, ...
        'Location', 'best', 'FontName', 'SimSun', 'FontSize', 16);
    ...
    % 保存图片（高分辨率，PNG + EMF）
    base_name = sprintf('rmse_comparison_%s', extractBefore(filename, '.csv'));
    set(gcf, 'PaperPositionMode', 'auto');
    print(gcf, fullfile(output_dir, [base_name, '.png']), '-dpng', '-r600');
    print(gcf, fullfile(output_dir, [base_name, '.emf']), '-dmeta', '-r600');
    
    % 输出RMSE数值到控制台
    fprintf('  东向RMSE - 元学习: %.4f, 零气动力: %.4f, 线性阻力: %.4f, 纯惯导: %.4f\n', ...
        rmse_meta_x, rmse_baseline_x, rmse_linear_x, rmse_pure_x);
    fprintf('  北向RMSE - 元学习: %.4f, 零气动力: %.4f, 线性阻力: %.4f, 纯惯导: %.4f\n', ...
        rmse_meta_y, rmse_baseline_y, rmse_linear_y, rmse_pure_y);
    fprintf('  天向RMSE - 元学习: %.4f, 零气动力: %.4f, 线性阻力: %.4f, 纯惯导: %.4f\n', ...
        rmse_meta_z, rmse_baseline_z, rmse_linear_z, rmse_pure_z);
    fprintf('完成文件: %s\n\n', filename);
