...
torch.set_default_tensor_type('torch.DoubleTensor') # 设置张量的默认类型为双精度浮点数，即64位浮点数
from torch.utils.data import Dataset, DataLoader # 导入这两个类，前者用于封装数据集，后者用于创建可迭代的数据加载器，可以批量加载数据，支持多线程加载
from torch.utils.data.dataset import random_split # 导入这个函数，可将数据集随机分割为多个子集
...
for my_a in range(3,4):
    ...
    for i in range(options['num_c']):
        ...
        trainloader = torch.utils.data.DataLoader(trainset, batch_size=options['phi_shot'], shuffle=options['shuffle'],
                                                  num_workers=NUM_WORKERS)
        adaptloader = torch.utils.data.DataLoader(adaptset, batch_size=options['K_shot'], shuffle=options['shuffle'],
                                                  num_workers=NUM_WORKERS)
    ...
    for epoch in range(options['num_epochs']):
        ...
        with torch.no_grad():
            for j in range(len(TestData)):
                loss_nominal, loss_mean, loss_phi = mlmodel.error_statistics(TestData[j].X, TestData[j].Y, phi_net, h_net,
                                                                             options=options)
